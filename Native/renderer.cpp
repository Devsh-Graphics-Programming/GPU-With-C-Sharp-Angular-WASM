...
// loads using file system - does not work if compiled with emscripten
GLuint load_cubemap_from_file(std::string faces[], std::string& env_path)
{
	GLuint textureID;
	glGenTextures(1, &textureID);
	glBindTexture(GL_TEXTURE_CUBE_MAP, textureID);
	int m_window_width, m_window_height, nrChannels;
	for (unsigned int i = 0; i < 6; i++)
	{
		unsigned char* data = stbi_load((env_path + faces[i]).c_str(), &m_window_width, &m_window_height, &nrChannels, 0);
		if (data)
		{
			glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i,
				0, GL_RGB, m_window_width, m_window_height, 0, GL_RGB, GL_UNSIGNED_BYTE, data);
...
void Renderer::init_cubemaps() {
	...
	{
		m_cubemaps[i] = load_cubemap_from_file(files[i], storage_folder_dir);
	}
}
...
{
	std::ifstream file("../../data/" + std::string(shaderPath));
